 ...
 * Wraps MegaLLM via OpenAI-compatible SDK for use with the provider facade.
 */
...
/**
 * Generate style directions using MegaLLM (non-streaming).
 * Returns an array of style/direction names.
 */
export async function megaGenerateStyles(options: GenerateStylesOptions): Promise<string[]> {
...
/**
 * Stream HTML artifact generation using MegaLLM.
...
/**
 * Stream React component conversion using MegaLLM.
 * Converts HTML/CSS to a React functional component.
 */
export async function* megaStreamReactComponent(
  options: StreamReactComponentOptions
): AsyncGenerator<string, void, unknown> {
  const { html, modelId = DEFAULT_MEGALLM_MODEL } = options;

  const client = getMegaLLMClient();

  const systemPrompt =
    'You are an expert React developer. Output ONLY the React component code without markdown code fences. No explanations.';
...
/**
 * Stream snippet extraction using MegaLLM.
 * Extracts a selected element into a standalone HTML file.
 */
export async function* megaStreamSnippetExtraction(
  ...
  const systemPrompt =
    'You extract HTML elements into standalone files. Output ONLY raw HTML without markdown code fences.';
...
/**
 * Stream snippet to React conversion using MegaLLM.
 */
export async function* megaStreamSnippetToReact(
  options: StreamSnippetToReactOptions
): AsyncGenerator<string, void, unknown> {
  const { snippetHtml, modelId = DEFAULT_MEGALLM_MODEL } = options;
...
/**
 * Stream variations generation using MegaLLM.
 * Generates conceptual variations of a design.
 */
export async function* megaStreamVariations(
  ...
  const systemPrompt =
    'You are a creative UI designer. Output ONLY valid JSON without markdown code fences.';
